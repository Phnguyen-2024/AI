...
#     # Initialize total_time to track execution time
#     start_time = time.time()
#     total_time = 0.0  # Ensure total_time is always defined
...
#         total_time = time.time() - start_time  # Calculate total_time here
...
# def get_state_from_entries(entries):
#     """Get the 3x3 puzzle state from entry fields."""
#     numbers = []
#     state = []
#     for i in range(3):
#         row = []
...
# def update_view(state):
#     """Update the visual representation of the puzzle state."""
#     for i in range(3):
#         for j in range(3):
#             tile = view_labels[i][j]
#             value = state[i][j]
...
# def create_state_grid(parent, state_3x3):
#     """Create a 3x3 grid to display a puzzle state."""
#     for i in range(3):
#         for j in range(3):
#             val = state_3x3[i][j]
#             bg_color = "#E3F2FD" if val == 0 else "#FFFFFF"
...
# def add_step_frame(index, move_, state_3x3):
#     """Add a frame to display a step in the solution."""
#     global step_count, current_row_frame
#     if step_count % 5 == 0:
#         current_row_frame = tk.Frame(steps_container, bg="#FAFAFA")
#         current_row_frame.pack(side="top", padx=5, pady=5)
...
# def pause_program():
#     """Pause the animation of the solution steps."""
#     global paused
#     paused = True

# def resume_program():
#     """Resume the animation of the solution steps."""
#     global paused
#     paused = False
...
# def update_comparison_table(algorithm_name, steps, total_time):
#     """Update the comparison table with algorithm performance."""
#     global compare_tree, current_group
#     algo_group = "uninformed"
#     if algorithm_name in ["a_star", "ida_star", "greedy"]:
#         algo_group = "informed"
...
# def solve(algorithm_name):
#     """Solve the 8-puzzle using the selected algorithm."""
#     global result, paused, running, step_count, current_row_frame
#     if not algorithm_name:
#         messagebox.showwarning("Selection Error", "Please select an algorithm!")
#         return
...
#     # Check solvability with a fallback
#     try:
#         if not solver.is_solvable(initial_state):
#             messagebox.showerror("Error", "Initial state is not solvable!")
#             running = False
#             return
#     except AttributeError:
#         # Fallback solvability check if solver lacks is_solvable
#         flat_state = [num for row in initial_state for num in row if num != 0]
...
#         if solution is not None:
#             try:
#                 final_state = tuple(tuple(row) for row in solver.apply_moves(initial_state, solution))
#                 if final_state == goal_state:
#                     verification_result = "Correct"
#                     add_step_frame(0, "Start", initial_state)
#                     add_step_frame(1, "Solved", final_state)
#                     update_view(final_state)
#                     steps_count.config(text=f"Số bước: {steps}")
#                     time_label.config(text=f"Thời gian: {total_time:.2f}s")
#                 else:
#                     verification_result = "Incorrect"
#                     messagebox.showinfo("Result", "Solution does not match goal state!")
...
#     else:
#         if algorithm_name == "partially_observable_search":
#             initial_percept = solver.observe(initial_state)
#             solution = algorithm(initial_percept)
#         else:
#             solution = algorithm(initial_state)
#         steps = len(solution) if solution else 0
#         total_time = time.time() - start_time
...
def solve(algorithm_name):
    ...
    if algorithm_name == "csp_ac3":
        ...
        if algorithm_name == "partially_observable_search":
            # Use a default noise probability if noise_scale is not defined
            noise_prob = 0.1  # Default value
            initial_percept = solver.observe(initial_state, noise_probability=noise_prob)
            solution = algorithm(initial_percept, noise_probability=noise_prob)
        else:
            solution = algorithm(initial_state)
...
#     global result, paused, running, step_count, current_row_frame
#     if not algorithm_name:
#         messagebox.showwarning("Lỗi chọn thuật toán", "Vui lòng chọn một thuật toán!")
#         return
#     running = True
#     step_count = 0
#     current_row_frame = None
...
#         steps = 1 if solution else 0
#         total_time = time.time() - start_time
#         time_label.config(text=f"Thời gian: {total_time:.2f}s")  # Cập nhật thời gian ngay lập tức
#         if solution is not None:
#             final_state = solution
#             if final_state == goal_state:
#                 verification_result = "Đúng"
#                 add_step_frame(0, "Bắt đầu", initial_state)
#                 add_step_frame(1, "Đã giải", final_state)
#                 update_view(final_state)
#                 steps_count.config(text="Số bước: 1 (CSP)")
#             else:
#                 verification_result = "Sai"
#                 messagebox.showinfo("Kết quả", "Giải pháp không khớp với trạng thái mục tiêu!")
#                 steps_count.config(text="Số bước: 1 (CSP)")
#         else:
#             verification_result = "Không có giải pháp"
#             steps_count.config(text="Số bước: 0")
#             messagebox.showinfo("Kết quả", "Không tìm thấy giải pháp!")
#         verification_label.config(text=f"Kiểm tra: {verification_result}")
#     else:
#         if algorithm_name == "partially_observable_search":
#             noise_prob = 0.1
#             initial_percept = solver.observe(initial_state, noise_probability=noise_prob)
...
#                     verification_result = "Sai"
#             except ValueError as e:
#                 verification_result = "Di chuyển không hợp lệ"
#                 messagebox.showerror("Lỗi", f"Di chuyển không hợp lệ trong giải pháp: {e}")
#                 solution = None
#         else:
#             verification_result = "Không có giải pháp"
#         verification_label.config(text=f"Kiểm tra: {verification_result}")
...
#         if solution:
#             current_state = initial_state
#             add_step_frame(0, "Bắt đầu", current_state)
#             for i, move_ in enumerate(solution, start=1):
#                 if not running:
#                     break
#                 while paused and running:
...
#                     add_step_frame(i, move_, new_state)
#                 except ValueError as e:
#                     messagebox.showerror("Lỗi", f"Di chuyển không hợp lệ {move_}: {e}")
#                     break
#             if running:
#                 steps_count.config(text=f"Số bước: {len(solution)}")
#         else:
