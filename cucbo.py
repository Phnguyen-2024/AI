...
#     def simple_hill_climbing(self, start, max_iterations=1000):
...
#     def steepest_ascent_hill_climbing(self, start, max_iterations=1000):
#         if start == self.goal_state:
#             return []
#         current = start
#         path = []
...
#             temp *= cooling_rate
#             iterations += 1
#         return best_path if best == self.goal_state else None
...
#             final_state = self.apply_moves(start, moves)
#             h = self.heuristic(final_state)
#             length_penalty = len(moves) * 0.05  # Reduced penalty
#             return -h - length_penalty
